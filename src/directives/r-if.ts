...
let directiveQueue: Array<{ key: DIRECTIVE; value: true | false | undefined }> = []
...
export default (path: NodePath<JSXElement>) => {
  ...
  if (directiveName) {
    ...
    if (directiveName !== DIRECTIVE.IF) {
      // 获取队列最后一个指令
      const prevDirective = directiveQueue[directiveQueue.length - 2]
      // r-else-if
